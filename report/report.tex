...
Il software coffeBreak è una versione semplificata del classico gioco arcade "Donkey Kong" del 1981. L'obiettivo 
del giocatore è controllare Mario, evitando ostacoli e raggiungendo la cima del livello per salvare Pauline 
dalle grinfie di Donkey Kong.

Ogni livello presenta piattaforme, scale e barili lanciati da Donkey Kong, che il giocatore deve 
schivare o saltare per avanzare. Il gioco si articola in più round con difficoltà crescente, introducendo 
nuovi ostacoli e velocità incrementale. 
Il punteggio aumenta in base al numero di ostacoli evitati e ai livelli completati.

\subsection*{Requisiti funzionali}
\begin{itemize}
	\item Il giocatore può controllare Mario con movimenti sinistra, destra, salto e salita/discesa scale.
	\item Il livello è completato quando Mario raggiunge Pauline.
	\item Donkey Kong lancerà barili a intervalli regolari con traiettorie variabili.
	\item Il gioco presenta punti accumulati per ogni ostacolo superato e bonus per la velocità di completamento.
	\item La difficoltà aumenta progressivamente con nuovi pattern di lancio di barili, nemici e ostacoli dinamici.
	\item Implementazione di un sistema di vite: il giocatore perde una vita se viene colpito da un barile.
\end{itemize}
\subsection*{Requisiti funzionali}
\begin{itemize}
	\item Il gioco deve avere una grafica retro in pixel art con animazioni fluide.
	\item Deve supportare un frame rate stabile per garantire una buona esperienza di gioco.
	\item Deve essere compatibile su più sistemi operativi (Windows, Mac, Linux).
	\item Il codice deve essere modulare e facilmente estendibile per future implementazioni.
\end{itemize}

\section{Modello del Dominio}

Il gioco è strutturato in una serie di round, ognuno dei quali presenta un layout 
differente di piattaforme. Le piattaforme sono superfici su cui il personaggio, 
controllato dal giocatore, può camminare, saltare e interagire. In aggiunta, sono presenti 
delle scale che il personaggio deve salire per completare ogni round.

Il giocatore controlla un personaggio in un contesto simile a quello di Donkey Kong, con 
l'obiettivo di evitare i barili e di raggiungere Pauline. Durante il 
percorso, il personaggio può raccogliere power-up e monete che incrementano il punteggio.

All'avvio del gioco, viene mostrato un menu iniziale che consente di avviare la partita 
o uscire dal gioco. Una volta avviata la partita, il personaggio può essere controllato 
tramite input da tastiera, e interagirà con gli oggetti e gli elementi presenti nel gioco.

In caso di sconfitta, il giocatore avrà la possibilità di tornare al menu principale e 
avviare una nuova partita. Il punteggio ottenuto sarà visibile in un'area designata dello 
schermo, e aumenterà progressivamente man mano che il gioco prosegue. Inoltre, sarà 
possibile mettere il gioco in pausa e riprenderlo quando desiderato.

Un aspetto importante del gioco è che il personaggio, Mario, perderà una vita se dovesse cadere da una grande altezza, aggiungendo un ulteriore livello di difficoltà e di attenzione nella gestione dei salti e dei movimenti.
...
L'architettura di CoffeeBreak segue il pattern architetturale MVC. Questo pattern è studiato 
per suddividere i compiti fra il Modello, Controller e View, permettendo di 
lavorare in settori distinti. Grazie a questa struttura, è possibile modificare 
un determinato settore senza stravolgere l'intero sistema. Ad esempio, l'aspetto grafico 
può essere migliorato senza impattare sulla logica del gioco.
...
All'avvio del gioco, verrà presentato un menu principale, dal quale sarà possibile iniziare una nuova partita premendo il tasto \textbf{ENTER} oppure chiudere l'applicativo premendo il tasto \textbf{Q}.

Durante il gioco, i movimenti del personaggio saranno controllati tramite le frecce direzionali:

\begin{itemize}
    \item \textbf{FRECCIA SU}: consente di salire una scala quando ci si trova nelle vicinanze.
    \item \textbf{FRECCIA GIÙ}: permette di scendere una scala quando ci si trova nelle vicinanze.
    \item \textbf{FRECCIA DESTRA}: sposta il personaggio verso destra.
    \item \textbf{FRECCIA SINISTRA}: sposta il personaggio verso sinistra.
    \item \textbf{ESC}: mette il gioco in pausa.
\end{itemize}
%
Quando il gioco è in pausa, i controlli sono i seguenti:

\begin{itemize}
    \item \textbf{FRECCIA SU}: sposta la selezione verso l'alto.
    \item \textbf{FRECCIA GIÙ}: sposta la selezione verso il basso.
    \item \textbf{ENTER}: seleziona l'opzione evidenziata.
\end{itemize}
%
In caso di Game Over, sarà possibile, premere \textbf{ENTER}, per tornare al menù principale. 
Buona fortuna! Ricorda di salvare la damigella prima che sia troppo tardi!
